...
"""
changed line 355 to stop the model from saving images
...
sys.path.append("~/iitb-racing-ros2/src/perception/perception/object_detect/yolov5/models/weights/last.pt")


from perception.object_detect.yolov5.models.common import DetectMultiBackend
from perception.object_detect.yolov5.utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
from perception.object_detect.yolov5.utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr,
                          increment_path, non_max_suppression, print_args, strip_optimizer, scale_segments, xyxy2xywh) 
from perception.object_detect.yolov5.utils.plots import Annotator, colors, save_one_box
from perception.object_detect.yolov5.utils.torch_utils import select_device, time_sync
from perception.object_detect.yolov5.utils.augmentations import letterbox
import numpy as np
...
class Yolo():
   ...
   def detect_all(self,
       ...
       #source = str(source)
       save_img = True #not nosave and not source.endswith('.txt')  # save inference images # true
       is_file = True #Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS) # true
       ...
       #for path, im, im0s, vid_cap, s in dataset:
       im0s = source
       assert im0s is not None, f'Image Not Found {source}'
       s = f'image {1}/{1} {source}: '
       ...
       for i, det in enumerate(pred):  # per image
           ...
           #p = Path(p)  # to Path
           save_path = str(save_dir / "random_run.png")  # im.jpg  # save_dir = object_detect/runs/detect
           txt_path = str(save_dir / 'labels' / "random_run") + (
               '' )  # im.txt
           s += '%gx%g ' % im.shape[2:]  # print string
           gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
           imc = im0.copy() if save_crop else im0  # for save_crop
           ...
           if len(det):
               ...
               for *xyxy, conf, cls in reversed(det):
                   ...
                   if save_img or save_crop or view_img:  # Add bbox to image
                       ...
                       annotator.box_label(xyxy, label, color=colors(c, True))
                       if save_crop:
                           save_one_box(xyxy, imc, file=save_dir / 'crops' / self.names[c] / f'{"random_run"}.jpg', BGR=True)
    ...
    #    LOGGER.info(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {(1, 3, *(self.imgsz))}' % t)
       if save_txt or save_img and False:
